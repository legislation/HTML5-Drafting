...
(function() {
  ...
  define(['aloha', 'block/block', 'block/blockmanager', 'aloha/plugin', 'aloha/pluginmanager', 'jquery', 'aloha/ephemera', 'ui/ui', 'ui/button', 'copy/copy-plugin'], function(Aloha, Block, BlockManager, Plugin, pluginManager, jQuery, Ephemera, UI, Button, Copy) {
    var DIALOG_HTML, activate, bindEvents, blockControls, blockDragHelper, blockIdentifier, blockTemplate, cleanIds, cleanWhitespace, copyBuffer, deactivate, getLabel, getType, insertElement, pluginEvents, registeredTypes, semanticInline, settings, topControls;
    if (pluginManager.plugins.semanticinline) {
      return pluginManager.plugins.semanticinline;
    }
    semanticInline = Block.AbstractBlock.extend({
      shouldDestroy: function() {
        return false;
      }
    });
    BlockManager.registerBlockType('semanticInline', semanticInline);
    settings = {};
    ...
    blockTemplate = jQuery('<span class="semantic-container aloha-ephemera-wrapper"></span>');
    topControls = jQuery('');
      ...
      if (!($element.is('.semantic-container') || ($element.is('.alternates') && $element.parents('figure').length))) {
        ...
        if (!type) {
          $element.wrap(blockTemplate).parent().append(controls).alohaBlock({
            'aloha-block-type': 'semanticInline'
          });
        } else {
          if (type.options) {
        ...
        return jQuery('<div>').addClass('').appendTo($element).aloha().append($body);
      }
    };
    ...
    return Plugin.create('semanticinline', {
